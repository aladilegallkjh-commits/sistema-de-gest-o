...
function DashboardContent({ summary, reports, projects, setActive }: { summary: any; reports: any; projects: typeof demoProjects; setActive: (id: string) => void }) {
  ...
  return <><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{metrics.map(metric => { const Icon = metric.icon; return <Card key={metric.label} className="group overflow-hidden rounded-2xl border-0 bg-white shadow-[0_8px_28px_rgba(25,45,90,0.05)] transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"><CardContent className="relative p-5"><div className={`absolute right-0 top-0 h-24 w-24 translate-x-8 -translate-y-8 rounded-full opacity-10 transition-transform duration-500 group-hover:scale-110 ${metric.color === "zinc" ? "bg-zinc-800" : metric.color === "violet" ? "bg-violet-500" : metric.color === "emerald" ? "bg-emerald-500" : "bg-amber-500"}`} /><div className="mb-5 flex items-start justify-between"><p className="text-xs font-medium text-[#8792a8]">{metric.label}</p><div className={`rounded-xl p-2 transition-transform duration-300 group-hover:rotate-6 ${metric.color === "zinc" ? "bg-zinc-100 text-zinc-800" : metric.color === "violet" ? "bg-violet-50 text-violet-600" : metric.color === "emerald" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}><Icon className="h-4 w-4" /></div></div><p className="text-[28px] font-semibold tracking-[-0.05em] text-[#172033]">{metric.value}</p><p className="mt-2 text-[11px] font-medium text-emerald-600">{metric.detail}</p></CardContent></Card> })}</section><section className="mt-7 grid gap-6 xl:grid-cols-[1.35fr_1fr]"><Card className="rounded-2xl border-0 bg-white shadow-[0_8px_28px_rgba(25,45,90,0.05)]"><CardHeader className="flex flex-row items-center justify-between space-y-0 border-b border-[#eef1f6] px-6 py-5"><div><CardTitle className="text-base font-semibold">Central de atenção</CardTitle><p className="mt-1 text-xs text-[#96a0b2]">Prioridades que pedem uma ação hoje</p></div><Badge variant="outline" className="rounded-full border-rose-100 bg-rose-50 text-rose-600">6 alertas</Badge></CardHeader><CardContent className="p-0">{alerts.map((alert, index) => { const Icon = alert.icon; return <button onClick={() => setActive(alert.type === "Estoque" ? "stock" : alert.type === "Cliente" ? "post-sale" : alert.type === "Atenção" ? "costs" : "production")} key={alert.type} className="group flex w-full items-center gap-4 border-b border-[#f0f2f6] px-6 py-4 text-left last:border-0 hover:bg-[#fbfcfe] transition-colors"><div className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl transition-transform duration-300 group-hover:scale-105 ${alert.tone === "rose" ? "bg-rose-50 text-rose-500" : alert.tone === "amber" ? "bg-amber-50 text-amber-500" : alert.tone === "cyan" ? "bg-cyan-50 text-cyan-600" : "bg-emerald-50 text-emerald-600"}`}><Icon className="h-[18px] w-[18px]" /></div><div className="min-w-0 flex-1"><div className="mb-1 flex items-center gap-2"><span className={`text-[10px] font-bold uppercase tracking-wider ${alert.tone === "rose" ? "text-rose-500" : alert.tone === "amber" ? "text-amber-500" : alert.tone === "cyan" ? "text-cyan-600" : "text-emerald-600"}`}>{alert.type}</span><span className="h-1 w-1 rounded-full bg-[#d7dce6]" /><p className="truncate text-sm font-medium text-[#273249]">{alert.title}</p></div><p className="truncate text-xs text-[#97a1b2]">{alert.text}</p></div><span className="hidden items-center gap-1 text-xs font-semibold text-zinc-800 opacity-0 transition group-hover:opacity-100 sm:flex translate-x-2 group-hover:translate-x-0">{alert.action}<ChevronRight className="h-3.5 w-3.5" /></span></button> })}</CardContent></Card><Card className="rounded-2xl border-0 bg-white shadow-[0_8px_28px_rgba(25,45,90,0.05)] transition-all duration-300 hover:shadow-lg"><CardHeader className="flex flex-row items-center justify-between space-y-0 px-6 pb-0 pt-5"><div><CardTitle className="text-base font-semibold">Ritmo de produção</CardTitle><p className="mt-1 text-xs text-[#96a0b2]">Ordens concluídas nesta semana</p></div><div className="rounded-lg bg-emerald-50 px-2 py-1 text-[11px] font-semibold text-emerald-600">+18,6%</div></CardHeader><CardContent className="px-4 pb-5 pt-6"><div className="h-[180px] w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={weekly} barSize={22}><CartesianGrid vertical={false} stroke="#eef1f6" /><XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: "#9aa3b5", fontSize: 11 }} /><YAxis hide /><Tooltip cursor={{ fill: "#f6f8fc" }} contentStyle={{ border: "0", borderRadius: "12px", boxShadow: "0 8px 24px rgba(25,45,90,.12)", fontSize: 12 }} /><Bar dataKey="value" radius={[6, 6, 2, 2]} fill="#27272a" /></BarChart></ResponsiveContainer></div></CardContent></Card></section><section className="mt-7 grid gap-6 xl:grid-cols-[1.35fr_1fr]"><Card className="rounded-2xl border-0 bg-white shadow-[0_8px_28px_rgba(25,45,90,0.05)] transition-all duration-300 hover:shadow-lg"><CardHeader className="flex flex-row items-center justify-between space-y-0 border-b border-[#eef1f6] px-6 py-5"><div><CardTitle className="text-base font-semibold">Projetos em andamento</CardTitle><p className="mt-1 text-xs text-[#96a0b2]">Acompanhe a saúde da sua carteira</p></div><Button variant="ghost" onClick={() => setActive("commercial")} className="gap-1 rounded-lg text-xs font-semibold text-zinc-800 transition-transform hover:translate-x-1">Ver todos <ChevronRight className="h-3.5 w-3.5" /></Button></CardHeader><CardContent className="p-0">{projects.map(project => <div key={project.code} className="flex flex-col gap-3 border-b border-[#f0f2f6] px-6 py-4 last:border-0 sm:flex-row sm:items-center group hover:bg-[#fbfcfe] transition-colors"><div className="flex min-w-0 flex-1 items-center gap-3"><div className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl transition-transform duration-300 group-hover:scale-105 ${project.tone === "blue" ? "bg-zinc-100 text-zinc-800" : project.tone === "violet" ? "bg-violet-50 text-violet-600" : "bg-emerald-50 text-emerald-600"}`}><BriefcaseBusiness className="h-4 w-4" /></div><div className="min-w-0"><div className="flex items-center gap-2"><span className="text-[10px] font-bold text-[#8a95aa]">{project.code}</span><span className="h-1 w-1 rounded-full bg-[#dce1eb]" /><p className="truncate text-sm font-semibold text-[#273249]">{project.name}</p></div><p className="mt-0.5 truncate text-xs text-[#9aa3b5]">{project.client}</p></div></div><div className="flex w-full items-center gap-4 sm:w-[240px]"><div className="flex-1"><div className="mb-1.5 flex justify-between text-[10px] font-medium"><span className="text-[#8290a5]">{project.status}</span><span className="text-[#273249]">{project.progress}%</span></div><Progress value={project.progress} className="h-1.5 bg-[#edf0f6]" /></div><div className="hidden text-right sm:block"><p className="text-xs font-semibold text-[#273249]">{project.value}</p><p className="mt-0.5 text-[10px] text-emerald-600">{project.margin} margem</p></div></div></div>)}</CardContent></Card><Card className="relative overflow-hidden rounded-2xl border-0 bg-[#172033] text-white shadow-[0_8px_28px_rgba(25,45,90,0.14)] transition-all duration-300 hover:shadow-xl"><div className="absolute -right-20 -top-20 h-40 w-40 rounded-full bg-white/5 blur-3xl pointer-events-none" /><CardHeader className="px-6 pb-0 pt-5 relative z-10"><CardTitle className="text-base font-semibold">Distribuição da carteira</CardTitle><p className="mt-1 text-xs text-white/45">Projetos por etapa operacional</p></CardHeader><CardContent className="flex items-center gap-5 px-4 py-5 relative z-10"><div className="h-[180px] w-[180px] shrink-0 transition-transform duration-500 hover:scale-105"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={stageData} dataKey="value" nameKey="name" innerRadius={54} outerRadius={75} paddingAngle={4} stroke="none">{stageData.map(entry => <Cell key={entry.name} fill={entry.color} className="transition-all hover:opacity-80" />)}</Pie><Tooltip contentStyle={{ border: "0", borderRadius: "12px", color: "#172033", fontSize: 12, boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }} /></PieChart></ResponsiveContainer></div><div className="min-w-0 flex-1 space-y-3">{stageData.map(item => <div key={item.name} className="group flex items-center justify-between text-xs transition-colors hover:bg-white/5 rounded-md p-1 -ml-1"><span className="flex items-center gap-2 text-white/60 group-hover:text-white/80"><span className="h-2 w-2 rounded-full transition-transform group-hover:scale-125" style={{ background: item.color }} />{item.name}</span><strong className="font-semibold text-white">{item.value}%</strong></div>)}<div className="mt-4 border-t border-white/10 pt-3"><p className="text-[11px] leading-relaxed text-white/45">A carteira mantém um equilíbrio saudável entre produção e relacionamento.</p></div></div></CardContent></Card></section></>;
}
...
function ModulePlaceholder({ active, onBack }: { active: string; onBack: () => void }) {
  ...
  return <div className="space-y-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-xl bg-zinc-100 text-zinc-800"><Icon className="h-5 w-5" /></div><div><h2 className="text-2xl font-semibold tracking-tight">{item.label}</h2><p className="mt-1 text-sm text-[#8792a8]">{data.intro}</p></div></div></div><div className="flex gap-2"><Button variant="outline" onClick={onBack} className="rounded-xl">Voltar</Button><Button className="gap-2 rounded-xl bg-zinc-800 hover:bg-zinc-900"><Plus className="h-4 w-4" /> Novo registro</Button></div></div><div className="grid gap-4 sm:grid-cols-3">{data.stats.map(([label, value]) => <Card key={label} className="rounded-2xl border-0 bg-white shadow-soft"><CardContent className="p-5"><p className="text-xs text-[#8792a8]">{label}</p><p className="mt-3 text-2xl font-semibold tracking-tight">{value}</p><p className="mt-2 text-[11px] font-medium text-emerald-600">Atualizado agora</p></CardContent></Card>)}</div><Card className="overflow-hidden rounded-2xl border-0 bg-white shadow-soft"><CardHeader className="flex flex-row items-center justify-between border-b border-[#eef1f6] px-6 py-5"><div><CardTitle className="text-base">Acompanhamento operacional</CardTitle><p className="mt-1 text-xs text-[#96a0b2]">Registros mais recentes do módulo</p></div><Button variant="outline" size="sm" onClick={exportCsv} className="hidden rounded-lg sm:flex">Exportar CSV</Button></CardHeader><CardContent className="p-0">{liveRows.map(([code, title, status]) => <div key={code} className="flex items-center gap-4 border-b border-[#f0f2f6] px-6 py-4 last:border-0"><div className="grid h-9 w-9 place-items-center rounded-xl bg-zinc-100 text-zinc-800"><FileText className="h-4 w-4" /></div><div className="min-w-0 flex-1"><p className="text-[10px] font-bold uppercase tracking-wider text-[#97a1b2]">{code}</p><p className="truncate text-sm font-semibold text-[#273249]">{title}</p></div><Badge variant="outline" className={`rounded-full border-0 text-[11px] ${status.includes("atras") || status.includes("desvio") || status.includes("Repor") ? "bg-rose-50 text-rose-600" : status.includes("hoje") || status.includes("Pendente") ? "bg-amber-50 text-amber-600" : "bg-emerald-50 text-emerald-600"}`}>{status}</Badge><ChevronRight className="h-4 w-4 text-[#c1c8d5]" /></div>)}</CardContent></Card></div>;
}

function ModuleSkeleton({ label, onBack }: { label: string; onBack: () => void }) {
  return <div className="space-y-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><div className="flex items-center gap-3"><Skeleton className="h-11 w-11 rounded-xl" /><div><h2 className="text-2xl font-semibold tracking-tight">{label}</h2><Skeleton className="mt-2 h-4 w-64" /></div></div></div><div className="flex gap-2"><Button variant="outline" onClick={onBack} className="rounded-xl">Voltar</Button><Skeleton className="h-10 w-32 rounded-xl" /></div></div><div className="grid gap-4 sm:grid-cols-3">{[1, 2, 3].map(i => <Card key={i} className="rounded-2xl border-0 bg-white shadow-soft"><CardContent className="p-5"><Skeleton className="h-3 w-24" /><Skeleton className="mt-3 h-8 w-16" /><Skeleton className="mt-2 h-3 w-20" /></CardContent></Card>)}</div><Card className="overflow-hidden rounded-2xl border-0 bg-white shadow-soft"><CardHeader className="flex flex-row items-center justify-between border-b border-[#eef1f6] px-6 py-5"><div><Skeleton className="h-5 w-48" /><Skeleton className="mt-2 h-3 w-64" /></div><Skeleton className="h-8 w-24 rounded-lg hidden sm:block" /></CardHeader><CardContent className="p-0">{[1, 2, 3, 4, 5].map(i => <div key={i} className="flex items-center gap-4 border-b border-[#f0f2f6] px-6 py-4 last:border-0"><Skeleton className="h-9 w-9 rounded-xl shrink-0" /><div className="min-w-0 flex-1 space-y-2"><Skeleton className="h-3 w-16" /><Skeleton className="h-4 w-48" /></div><Skeleton className="h-6 w-20 rounded-full" /></div>)}</CardContent></Card></div>;
}
